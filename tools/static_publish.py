...
Cache-bust stamps (?v=<file hash>) are either applied before hashing, or
canonicalized (stripped) when hashing, so stamped vs unstamped HTML produce
the same appVersion / releaseVersion.
"""
...
def canonicalize_html_for_hash(html: str) -> str:
    """Strip cache-bust ?v= from local asset URLs so stamped == unstamped identity."""
    ...
    def repl(m: re.Match) -> str:
        ...
        return f"{prefix}{quote}{rel}{fragment}{quote}"

    return _ATTR_RE.sub(repl, html or "")
...
def _file_bytes_for_hash(path: Path) -> bytes:
    """Canonicalize HTML ?v= stamps; hash other files as-is."""
    ...
    if path.suffix.lower() in {".html", ".htm"}:
        try:
            ...
            return raw
        return canonicalize_html_for_hash(text).encode("utf-8")
    return raw
...
def compute_app_version(web_dir: Path) -> str:
    ...
    HTML is canonicalized (local ?v= stripped) so stamp-then-hash and
    hash-then-stamp produce the same identity.
    """
    web_dir = Path(web_dir)
    h = hashlib.sha256()
    ...
    for p in files:
        ...
        _feed(h, _file_bytes_for_hash(p))
    return h.hexdigest()
