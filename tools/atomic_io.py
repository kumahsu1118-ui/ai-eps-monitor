...
Fail-closed: a crash mid-write must not leave a truncated public file, and
JSONL append never falls back to a non-atomic `open(..., "a")`.
...
def atomic_write_bytes(path: Path | str, data: bytes) -> Path:
    """Write `data` to `path` using flock + fsync + os.replace. No fallback."""
    path = Path(path)
...
class PipelineLock:
    ...
    def acquire(self, *, blocking: bool = False) -> None:
        ...
        # Record holder pid for diagnostics (advisory only).
        try:
            os.ftruncate(fd, 0)
